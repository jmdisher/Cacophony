...
public record EditPostCommand(IpfsFile _postToEdit, String _name, String _description, String _discussionUrl) implements ICommand<OnePost>
{
	...
	private static void _handleCacheUpdates(EntryCacheRegistry entryRegistry, LocalRecordCache recordCache, IpfsKey publicKey, IpfsFile oldCid, IpfsFile newCid, StreamRecord newStreamRecord)
	{
		...
		if (null != recordCache)
		{
			...
			recordCache.recordMetaDataPinned(newCid, newStreamRecord.getName(), newStreamRecord.getDescription(), newStreamRecord.getPublishedSecondsUtc(), newStreamRecord.getDiscussion(), newStreamRecord.getPublisherKey(), newStreamRecord.getElements().getElement().size());
			if (null != leaves.thumbnail)
			{
